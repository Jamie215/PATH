<script lang="ts">
  ...
  const name = new PatientNameField('phq4:patientName');
  ...
  async function downloadPDF(): Promise<void> {
    ...
    await pdf.run(async () => {
      ...
      const bytes = await generatePHQ4Report({ result: r, patientName: name.value });
...
{#if loaded && result}
  <div class="results">
    ...
    <div class="actions">
      ...
      <a href="/phq4/" data-clear-session class="btn btn--primary">Redo Assessment</a>
    </div>
    {#if pdf.error}
